...
def create_path(item, m_dir):
    ...
    new_path = os.path.join(m_dir, splited_dir[-2]) + '/' + splited_dir[-1][:-4] + '/' + item['fname'][:-4] + '.npy'
    return new_path
...
class ResampledShards(IterableDataset):
    ...
    def __iter__(self):
        ...
        if self.deterministic:
            ...
            seed = make_seed(self.worker_seed(), self.epoch, os.getpid(), time.time_ns(), os.urandom(4))
...
def create_webdataset(urls_path,
                      storage_options,
                      resolution=256,
                      tokenizer_name=None,
                      is_val=False,
    ...
    def preprocess_dataset(item):
        ...
        pil_image = pil_image_original.resize((768, 768), resample=Image.BICUBIC, reducing_gap=1)
        arr = np.array(pil_image.convert("RGB"))
        arr = arr.astype(np.float32) / 127.5 - 1
        if is_up:
            pil_image_low = pil_image_original.resize((low_res, low_res), resample=Image.BICUBIC,
                                                      reducing_gap=1)
...
def create_webdataset_loader(urls_path,
                             storage_options,
                             resolution=256,
                             tokenizer_name=None,
                             is_val=False,
    ...
    dataset = create_webdataset(urls_path=urls_path,
