...
class TextImageDataset(Dataset):
    def __init__(self, df_path,
                 tokenizer_name='M-CLIP/XLM-Roberta-Large-Vit-L-14', clip_image_size=224,
                 seq_len=77, drop_text_prob=0.5,
                 drop_image_prob=0.5, image_size=512,
                 infinity=False):
    ...
    def __getitem__(self, item):
        ...
        image = image.resize((self.image_size, self.image_size), resample=Image.BICUBIC, reducing_gap=1)
        ...
        if np.random.binomial(1, self.drop_image_prob):
            out_dict['clip_image'] = torch.zeros(3, self.clip_image_size, self.clip_image_size)
        else:
