...
class Decoder(pl.LightningModule):
    def __init__(
        ...
        if use_ema:
            self.ema_params = EMA(self.unet, ema_rate,)
    ...
    def create_image_encoder(self, ):
        if self.image_enc_params is not None:
            ...
            if self.image_enc_params['name'] == 'AutoencoderKL':
                ...
                self.image_encoder.load_state_dict(torch.load(self.image_enc_params['ckpt_path']))
    ...
    def configure_optimizers(self):
        optimizer = get_obj_from_str(self.optim_params['name'])(self.unet.parameters(), **self.optim_params['params'])
        lr_scheduler = get_obj_from_str(self.scheduler_params['name'])(optimizer, **self.scheduler_params['params'])
    ...
    def prepare_cond(self, cond):
        if self.use_text_enc:
            ...
            if 'mask' in cond:
                mask = cond['mask']
            with torch.no_grad():
                new_cond['image_emb'] = self.clip_model.encode_image(cond['clip_image']).float()
            with torch.no_grad():
