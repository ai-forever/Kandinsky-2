...
class Kandinsky2_1:
    ...
    def generate_img(self, prompt,
                     img_prompt, batch_size=1, diffusion=None,
                     guidance_scale=7, init_step=None, noise=None,
                     init_img=None, img_mask=None, h=512, w=512, sampler='ddim_sampler',
                     num_steps=50,
                     ):
        ...
        model_kwargs['full_emb'], model_kwargs['pooled_emb'] = self.encode_text(text_encoder=self.text_encoder,
        ...
        if sampler == 'p_sampler':
            ...
            self.model.del_cache()
            samples, _ = sampler.sample(num_steps, batch_size * 2, (4, new_h, new_w), conditioning=model_kwargs,
                                        x_T=noise, init_step=init_step)
            self.model.del_cache()
    ...
    def generate_text2img(self, prompt, num_steps=100,
                          batch_size=1, guidance_scale=7, h=512, w=512,
                          sampler='ddim_sampler', prior_cf_scale=4, prior_steps="25",
                          negative_prior_prompt='', negative_decoder_prompt=''):

        # generate clip embeddings
        image_emb = self.generate_clip_emb(prompt, batch_size=batch_size, prior_cf_scale=prior_cf_scale,
                                           prior_steps=prior_steps, negative_prior_prompt=negative_prior_prompt)
        if negative_decoder_prompt == '':
            zero_image_emb = self.create_zero_img_emb(batch_size=batch_size)
        else:
            zero_image_emb = self.generate_clip_emb(negative_decoder_prompt, batch_size=batch_size,
        ...
        return self.generate_img(prompt=prompt, img_prompt=image_emb,
                                 batch_size=batch_size,
                                 guidance_scale=guidance_scale,
                                 h=h, w=w, sampler=sampler, num_steps=num_steps,
                                 diffusion=diffusion)

    @torch.no_grad()
    def mix_images(self, images_texts, weights, num_steps=100,
                   batch_size=1, guidance_scale=7, h=512, w=512,
                   sampler='ddim_sampler', prior_cf_scale=4, prior_steps="25",
                   negative_prior_prompt='', negative_decoder_prompt=''):
        ...
        for i in range(len(images_texts)):
            if image_emb is None:
                if type(images_texts[i]) == str:
                    image_emb = self.generate_clip_emb(images_texts[i], batch_size=1, prior_cf_scale=prior_cf_scale,
                                                       prior_steps=prior_steps,
                                                       negative_prior_prompt=negative_prior_prompt) * weights[i]
                else:
                    image_emb = self.encode_images(images_texts[i], is_pil=True) * weights[i]
            else:
                if type(images_texts[i]) == str:
                    image_emb = image_emb + self.generate_clip_emb(images_texts[i], batch_size=1,
        ...
        if sampler == 'p_sampler':
            config['diffusion_config']['timestep_respacing'] = str(num_steps)
        diffusion = create_gaussian_diffusion(**config['diffusion_config'])
        return self.generate_img(prompt='', img_prompt=image_emb,
                                 batch_size=batch_size,
                                 guidance_scale=guidance_scale,
                                 h=h, w=w, sampler=sampler, num_steps=num_steps,
                                 diffusion=diffusion)
    ...
    def generate_img2img(self, prompt, pil_img, strength=0.7,
        ...
        return self.generate_img(prompt=prompt, img_prompt=image_emb,
                                 ...
                                 guidance_scale=guidance_scale,
                                 h=h, w=w, sampler=sampler, num_steps=num_steps,
                                 diffusion=diffusion, noise=image, init_step=start_step)
    ...
    def generate_inpainting(self, prompt, pil_img, img_mask,
        ...
        return self.generate_img(prompt=prompt, img_prompt=image_emb,
                                 ...
                                 guidance_scale=guidance_scale,
                                 h=h, w=w, sampler=sampler, num_steps=num_steps,
                                 diffusion=diffusion, init_img=image,
                                 img_mask=img_mask)